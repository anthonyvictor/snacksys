...
//             imageUrl: await contact.getProfilePicUrl(),
...
//     } else {
//       console.log(
//         "Nenhuma sessão WA encontrada. Gerando QR Code..",
//       );
//       client()
...
wa.on("message", async (msg: Message) => {
  ...
  if ([MessageTypes.AUDIO, MessageTypes.VOICE].includes(msg.type)) {
    await sleep(500);
    return msg.reply("Por favor, envie mensagem de texto.");
  }
...
export const sendWhatsappMessage = async (to: string, content: string) => {
  if (!isReady) {
console.log("WhatsApp não está pronto.")
    return
  };
  return wa.sendMessage(to, content);
};
