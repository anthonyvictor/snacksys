...
export const replaceProductsText = (text: string, generic = false) => {
  const r = text
    ...
    .replace(
      /\b(chori(c|s)a|(g|c)al?a?br?e(z|s)a|lingu?i(c|s)a)\b/g,
      generic ? "comida" : "calabresa",
    )
    .replace(/\b(a(c|s)+air?)\b/g, generic ? "comida" : "acai")
    .replace(
      /\b(p(a|e)?(s|x)?te(l|is|u|o|s))\b/g,
      generic ? "comida" : "pastel",
    )
    .replace(/\b(camar?a?o)\b/g, generic ? "comida" : "camarao")
    .replace(
      /\b(chic?ken|f(r|t)a?(n|m)?(g|f)(o|u)|galin?h?a)\b/g,
    ...
    .replace(
      /\b(a?p(r|t)?esu(n|m)?t(o|u|a?do|inho)?|misto)\b/g,
      generic ? "comida" : "presunto",
    )
